...
async def enter_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    private_key = "38b162ae7af095971c65824e767e1d312402f02030d2e47df9551d055d21ca6006652d69d5b97c24627d58bcb69bc217beabbc7b04c58b7cdcbbbaaa296b9d94"   #"YOUR_PRIVATE_KEY"  # Replace with your private key
    try:
        payload = {
            "from": sender_wallet,
            "to": wallet,
            "value": int(amount * 10**9),  # Convert to nanotokens
            "key": private_key,
        }
        response = requests.post(f"{API_URL}/sendTransaction", json=payload)

        if response.status_code == 200:
            data = response.json()
            transaction_hash = data.get("transaction_hash", "Unknown")
            await update.message.reply_text(
                f"🎉 Transaction successful!\n"
                f"Transaction Hash: {transaction_hash}"
            )
            # Log the transaction
            logs = context.bot_data.setdefault("logs", [])
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            user = update.message.from_user
            user_info = f"{user.username or 'Unknown'} ({user.id})"
            logs.append(
            ...
            await update.message.reply_text(
                f"❌ Transaction failed. Reason: {error_message}"
            )
    except Exception as e:
        await update.message.reply_text(
            f"❌ Transaction failed. Reason: {str(e)}"
        )
