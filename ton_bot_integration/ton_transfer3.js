...
async function fetchAccountState(client, address) {
    try {
        const accountState = await client.provider.getState(address);
...
async function sendTon(senderPrivateKeyHex, recipientAddress, amount) {
    try {
        ...
        // Step 5: Fetch seqno via direct RPC
        let seqno = 0;
        try {
            ...
            const accountState = await fetchAccountState(client, senderWalletAddress);
